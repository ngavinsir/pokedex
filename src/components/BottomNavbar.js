...
export function BottomNavbar() {
  return (
    <div tw="border-t-2 border-gray-300 py-1 bg-white">
      <div tw="px-8 flex justify-around max-w-lg mx-auto">
        <NavbarItem
          icon={
...
function NavbarItem({ icon, label, href }) {
  return (
    <Link to={href}>
      <div
        tw="flex flex-col items-center justify-center w-20 h-16 cursor-pointer"
        className="group"
      >
        {icon}
        <span tw="font-semibold text-sm text-gray-700 mt-1 can-hover:group-hover:text-blue-800">
          {label}
        </span>
      </div>
    </Link>
